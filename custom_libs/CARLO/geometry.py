...
class Point:
    ...
    def hasPassed(self, other: Union['Point', 'Line', 'Rectangle', 'Circle', 'Ring'],
...
class Line:
    ...
    def distanceTo(self, other: Union['Point', 'Line', 'Rectangle', 'Circle', 'Ring']) -> float:
        if isinstance(other, Point):
            ...
            return np.min([self.p1.distanceTo(other.p1), self.p1.distanceTo(other.p2),
                           self.p2.distanceTo(other.p1), self.p2.distanceTo(other.p2)])
...
class Rectangle:
    def __init__(self, c1: Point, c2: Point,
                 c3: Point):  # 3 points are enough to represent a rectangle
        self.c1 = c1
        self.c2 = c2
        self.c3 = c3
        self.c4 = c3 + c1 - c2
    ...
    def __str__(self):
        return 'Rectangle(' + str(self.c1) + ', ' + str(self.c2) + ', ' + str(self.c3) + ', ' + str(
            self.c4) + ')'
...
class Ring:
    ...
    def __str__(self):
        return 'Ring(' + str(self.m) + ', inner radius = ' + str(
            self.r_inner) + ', outer radius = ' + str(self.r_outer) + ')'
    ...
    def distanceTo(self, other: Union['Point', 'Line', 'Rectangle', 'Circle', 'Ring']) -> float:
        if isinstance(other, Point) or isinstance(other, Line) or isinstance(other,
                                                                             Rectangle) or isinstance(
            other, Circle):
            return other.distanceTo(self)
